...
- `|mᵢ|/‖w‖` gives the geometric distance to the decision boundary
...
5. **We start from w⁽⁰⁾ = 0**: Initial weights are zeros

We'll track how our learned weights w⁽ᵏ⁾ relate to the optimal weights w*.

#### The Two-Speed Race

We track two quantities as the perceptron makes mistakes:

1. **Alignment with optimal weights**: `w* · w⁽ᵏ⁾`
2. **Length of weight vector**: `‖w⁽ᵏ⁾‖`

The key insight: alignment grows linearly (∼ k) while length grows sublinearly (∼ √k). By the Cauchy-Schwarz inequality:

`w* · w⁽ᵏ⁾ ≤ ‖w*‖ · ‖w⁽ᵏ⁾‖`

Since the left side grows faster than the right, k must be bounded—guaranteeing convergence.

#### Speed 1: Alignment Grows Linearly

After a mistake on example (x, y) with y ∈ {±1}, the perceptron update:

```
w⁽ᵏ⁾ = w⁽ᵏ⁻¹⁾ + yx
```

The alignment with w* changes as:
...
Since w* achieves margin γ, for any point x with label y:
...
The alignment grows linearly with k.
...
Using the expansion ‖a + b‖² = ‖a‖² + 2(a · b) + ‖b‖²:

```
‖w⁽ᵏ⁾‖² = ‖w⁽ᵏ⁻¹⁾‖² + 2y(w⁽ᵏ⁻¹⁾ · x) + ‖x‖²
```

Since we only update on mistakes: `y(w⁽ᵏ⁻¹⁾ · x) ≤ 0`
...
If all examples satisfy ‖x‖ ≤ R:

After k mistakes: `‖w⁽ᵏ⁾‖² ≤ kR²`

Therefore: `‖w⁽ᵏ⁾‖ ≤ R√k`

The length grows as √k.

#### The Punchline

Combining our results with the Cauchy-Schwarz inequality:
...
The proof never actually constructs the separating hyperplane. We don't know what w* is, we just know it exists. The proof shows that our random walk through weight space (via mistakes) can't wander forever.
