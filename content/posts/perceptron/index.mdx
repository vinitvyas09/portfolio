...
Before we dive into the history and theory, let's play with what we're building toward. Below is a live perceptron - the exact same algorithm that will take us from biological inspiration to modern AI. Adjust the inputs and weights to see how this simple mathematical neuron makes decisions in real-time. Don't worry if it seems abstract now; by the end of this post, you'll understand every slider and number here.
...
<p className="text-sm text-gray-600 text-center mt-2 italic">Try it yourself: Adjust the inputs and weights to see how the perceptron makes decisions. We'll explain exactly what's happening here as we go.</p>

Now let's start from the beginning and build up to understanding what you just played with.
